...
function Nav() {
    ...
    return ( <div className = { `topnav ${scrolled}` } >
        ...
        <button className = "icon" onClick = { toggle } > { nav ? < i className = "fa fa-times" > </i> : <i className="fa fa-bars"></i > } </button> 
        {nav?
            ...
            smooth = 'easeInOutQuad' offset = {-70 } duration = { durationFn } > Activities </Link> 
            <Link to = "team" spy = { true } smooth = 'easeInOutQuad' offset = {-70 } duration = { durationFn } > Team </Link> { /* <Link to="gallery" spy={true} smooth='easeInOutQuad' offset={-70} duration={durationFn}>Gallery</Link> */ } 
            </div>:
                <div> </div>
        } </div>
    );
